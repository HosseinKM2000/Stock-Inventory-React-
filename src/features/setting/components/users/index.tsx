  ...
  if (users.isError)
    return (
      <Callout.Root color="red">
        <Callout.Text>دریافت کاربران ناموفق بود.</Callout.Text>
      </Callout.Root>
    );
              ...
              <Flex justify="between" align="start" gap="4" wrap="wrap">
                <Box className="min-w-0 flex-1">
                  ...
                  <Text as="div" size="2" color="gray" mt="2">
                    @{user.username} · {user.email || "بدون ایمیل"} ·{" "}
                    {user.phone || "بدون تلفن"}
                  </Text>
                  <Box
                    mt="3"
                    className="grid grid-cols-1 gap-1 text-sm sm:grid-cols-2 xl:grid-cols-4"
                  >
                    <Text>
                      اشتراک: {user.plan} (
                      {user.subscription_status === "active" ? "فعال" : "منقضی"}
                      )
                    </Text>
                ...
                <Flex direction="column" gap="2" className="w-full sm:w-60">
                  <Select.Root
                    ...
                    onValueChange={(planId) => {
                      const selected = (plans.data ?? []).find(
                        (plan) => plan.id === planId,
                      );
                      if (selected && planId !== user.plan)
                        setPendingAction({
                          kind: "subscription",
                          user,
                          plan: planId,
                  ...
                  >
                    <Select.Trigger
                      aria-label={`طرح اشتراک ${user.username}`}
                    />
                    <Select.Content>
                      {(plans.data ?? [])
                        .filter(
                          (plan) => plan.is_active || plan.id === user.plan,
                        )
                        .map((plan) => (
                  ...
                  <Flex gap="2" wrap="wrap">
                    ...
                    <Button
                      size="2"
                      variant="soft"
                      disabled={immutable || self || loading}
                      onClick={() =>
                        setPendingAction({
                          kind: "role",
                          user,
                    ...
                    >
                      {user.role === "ADMIN" ? "لغو مدیریت" : "اعطای مدیریت"}
                    </Button>
                    <Button
                      size="2"
