...
export function MobileNavigationDrawer() {
  ...
  const settingItems = settingsNavItems.filter(
    (item) => !("adminOnly" in item) || isAdmin(user),
  );

  const navigationLink = (item: {
    label: string;
    to: string;
    icon: typeof HamburgerMenuIcon;
  }) => {
  ...
  return (
    <DialogPrimitive.Root open={open} onOpenChange={setOpen}>
      ...
      <DialogPrimitive.Portal
        container={
          document.getElementById("app-theme-portal-root") ?? undefined
        }
      >
        ...
        >
          ...
          >
              ...
              <Box className="min-w-0">
                <DialogPrimitive.Title asChild>
                  <Text as="div" weight="bold" className="truncate">
                    {user ? `${user.first_name} ${user.last_name}` : "Tanzim"}
                  </Text>
                </DialogPrimitive.Title>
                <Text as="div" size="1" color="gray" className="truncate">
                  {user ? `${user.username}` : "مدیریت موجودی"}
                </Text>
              </Box>
            </Flex>
            <DialogPrimitive.Close asChild>
              <IconButton
                ...
                className="drawer-close-button shrink-0"
              >
                <span className="menu-toggle-icon is-open" aria-hidden="true">
                  <span />
                  <span />
                  <span />
                </span>
          ...
          <Box className="flex-1 overflow-y-auto overscroll-contain p-3">
            ...
            <Flex direction="column" gap="1">
              {primaryNavigationItems.map(navigationLink)}
            </Flex>
          ...
          <Box className="border-t border-foreground/10 p-4 pb-[calc(1rem+env(safe-area-inset-bottom))]">
            <Text size="1" color="gray">
              بسته شدن منو با استفاده از دکمه یا کلیک خارج از منو!
            </Text>
          </Box>
        </DialogPrimitive.Content>
      </DialogPrimitive.Portal>
